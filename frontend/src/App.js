...
const useResourceCounts = (k8sData) => {
  return useMemo(() => ({
    ...
    cronjobs: k8sData?.cronjobs?.length || 0,
    serviceaccounts: k8sData?.serviceaccounts?.length || 0,
    endpoints: k8sData?.endpoints?.length || 0,
    networkpolicies: k8sData?.networkpolicies?.length || 0,
    storageclasses: k8sData?.storageclasses?.length || 0,
    roles: k8sData?.roles?.length || 0,
    clusterroles: k8sData?.clusterroles?.length || 0,
    rolebindings: k8sData?.rolebindings?.length || 0,
    clusterrolebindings: k8sData?.clusterrolebindings?.length || 0,
    events: k8sData?.events?.length || 0,
  }), [k8sData]);
};
...
const AppContent = () => {
  ...
  const { getApiUrl, getCurrentClusterInfo } = useCluster();
  ...
  // State for managing tabs from sidebar
  const [sidebarOpenTab, setSidebarOpenTab] = useState(null);

  // Handle tab changes from sidebar
  const handleTabChange = useCallback((resourceType) => {
    setSidebarOpenTab(resourceType);
  }, []);

  // Render tab content based on active tab
  const renderTabContent = useCallback(({ activeTab, openTab }) => {
    // Handle sidebar tab opening
    if (sidebarOpenTab && openTab) {
      openTab(sidebarOpenTab);
      setSidebarOpenTab(null); // Reset after opening
    }
    if (isLoading) {
    ...
    switch (activeTab?.type) {
      ...
      case 'persistentvolumes':
        return (
          <Suspense fallback={<ModernLoadingFallback />}>
            <OptimizedResourceTable
              data={k8sData?.persistentvolumes || []}
      ...
      case 'persistentvolumeclaims':
        return (
          <Suspense fallback={<ModernLoadingFallback />}>
            <OptimizedResourceTable
              data={k8sData?.persistentvolumeclaims || []}
      ...
      case 'clusterrolebindings':
        return (
          <Suspense fallback={<ModernLoadingFallback />}>
            <OptimizedResourceTable
              data={k8sData?.clusterrolebindings || []}
  ...
  }, [k8sData, isLoading, selectedNamespace, resourceCounts, sidebarOpenTab]);
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white flex flex-col">
      ...
      <ModernAIChat
        ...
        k8sData={k8sData}
        currentCluster={getCurrentClusterInfo()}
      />
    </div>
  );
};
